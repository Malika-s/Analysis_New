...
4. **Upload .zip / .tar / .tar.gz** — drop a zipped or tarred lesson folder
   ...
   already loaded. If it finds both, it prefers the captions (more
   reliable) but still loads the video so timestamped issues are clickable.
...
Both checks always report their outcome explicitly — if nothing is found,
you'll see "no issues found" or "scan ran successfully, nothing flagged"
rather than an empty section, so it's clear the review actually ran.

### Coverage
Because the transcript is split into segments before Claude reviews it
(long lessons don't fit in one request), there's a **Coverage** section
showing exactly what was checked: how many segments total, the time range
each one covers, and whether it came back clean or with issues. If a
request fails partway through, the remaining segments are marked "not
checked — stopped early" instead of silently being skipped, so you always
know how much of the lesson was actually reviewed.
